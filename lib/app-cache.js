  ...
  _createClass(AppCache, [{
    ...
    value: function apply(plugin, compilation, compiler) {
      ...
      var cache = (this.caches.reduce(function (result, name) {
        ...
        return result;
      }, null) || []).map(pathRewrite).map(function (p) {
        return _path3['default'].posix.relative(_this.output, p);
      });
    ...
    value: function getManifestTemplate(cache, plugin) {
      ...
      if (!this.includeCrossOrigin) {
        cache = cache.filter(function (asset) {
          if ((0, _miscUtils.isAbsoluteURL)(asset) && (_this2.basePath === '/' || asset.indexOf(_this2.basePath) !== 0)) {
            return false;
          }
    ...
    value: function getPageContent() {
      if (this.events) {
        return _fs2['default'].readFileSync(_path3['default'].join(__dirname, '../tpls/appcache-frame.tpl'), 'utf-8');
      } else {
        return '';
      }
    }
